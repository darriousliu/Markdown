...
fun rememberMarkdownDocument(
    ...
    LaunchedEffect(markdown, isStreaming, parser, retainStateOnChange) {
        val currentState = if (retainStateOnChange) state else state.copy(document = null)
        state = updateStreamingDocumentState(
            ...
            parse = { value ->
                if (value.isEmpty()) null else withContext(Dispatchers.Default) { parser.parse(value) }
            },
        )
    }
